...
class ExternalSystemController(
    ...
    suspend fun process(
        ...
        if (Random.nextDouble(0.0, 1.0) < account.slo.fullBlockingProbability) {
            logger.error("Blocking account ${ blockList[accountName]}")
            blockList[accountName] = true
        }

        if (blockList[accountName] == true) {
            val waitDelay = Random.nextLong(account.slo.upperLimitInvocationMillis * 10)
            delay(waitDelay)
            blockList[accountName] = false
            logger.error("Unblocking account ${ blockList[accountName]} after $waitDelay delay")
        }
